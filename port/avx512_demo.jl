...
med(f) = (for _ in 1:20; f(); end; ts = Float64[]; for _ in 1:151; t = time_ns(); for _ in 1:20; f(); end; push!(ts, (time_ns()-t)/20); end; median(ts))
gf(n, t) = 5 * n * log2(n) / t
...
for (n, k) in zip(sizes, (1, 2, 3))
    ...
    m8 = med(() -> P.apply_unnormalized!(p8, b8)); m4 = med(() -> AR.applyplan!(r4, b4)); GC.@preserve rb (mr = med(() -> rpr(h, rb, n)))
...
plot!(p, sizes, g4; label = "PureFFT W=4 (AVX2, same tree)", color = :steelblue, lw = 2, marker = :circle, ms = 5)
